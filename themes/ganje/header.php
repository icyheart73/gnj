...
<a href="#main" class="sr-only sr-only-focusable"><?php _e( 'Skip to main content', 'ganje' ); ?></a>
	...
	<header>
		<nav id="header" class="navbar navbar-expand-md <?php echo $navbar_scheme; if ( isset( $navbar_position ) && 'fixed_top' === $navbar_position ) : echo ' fixed-top'; elseif ( isset( $navbar_position ) && 'fixed_bottom' === $navbar_position ) : echo ' fixed-bottom'; endif; if ( is_home() || is_front_page() ) : echo ' home'; endif; ?>">
			<div class="container">
				...
				<button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbar" aria-controls="navbar" aria-expanded="false" aria-label="<?php _e( 'Toggle navigation', 'ganje' ); ?>">
				...
				<div id="navbar" class="collapse navbar-collapse">
					...
					?>
							<form class="form-inline search-form my-2 my-lg-0" role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>">
								<input type="text" id="s" name="s" class="form-control mr-sm-2" placeholder="<?php _e( 'Search', 'ganje' ); ?>" title="<?php echo esc_attr( __( 'Search', 'ganje' ) ); ?>" />
								<button type="submit" id="searchsubmit" name="submit" class="btn btn-outline-secondary my-2 my-sm-0"><?php _e( 'Search', 'ganje' ); ?></button>
							</form>
					<?php
						endif;
					?>
