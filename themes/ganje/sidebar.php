	...
	<?php if ( is_archive() || is_single() ) : ?>
		...
		<div class="bg-faded sidebar-nav">
			<div id="primary-two" class="widget-area">
				<?php
					$output = '<ul class="recentposts">';
						...
						if ( $recentposts_query->have_posts() ) :
							$output .= '<li><h3>' . __( 'Recent Posts', 'ganje' ) . '</h3></li>';
							while ( $recentposts_query->have_posts() ) :
								...
								$output .= '<li>';
									...
									if ( $month !== $month_check ) :
										$output .= '<a href="' . esc_url( get_month_link( get_the_date( 'Y' ), get_the_date( 'm' ) ) ) . '" title="' . get_the_date( 'F, Y' ) . '">' . $month . '</a>';
									endif;
									$month_check = $month;

								$output .= '<h4><a href="' . esc_url( get_the_permalink() ) . '" title="' . sprintf( __( 'Permalink to %s', 'ganje' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark">' . get_the_title() . '</a></h4>';
								$output .= '</li>';
							endwhile;
						endif;
				...
				<br />
				<ul>
					<li><h3><?php _e( 'Categories', 'ganje' ); ?></h3></li>
					<?php
					...
					?>
							<li>&nbsp;</li>
							<li><a href="<?php echo esc_url( get_the_permalink( get_option( 'page_for_posts' ) ) ); ?>" class="btn btn-outline-secondary"><?php _e( 'more', 'ganje' ); ?></a></li>
					<?php
						endif;
					?>
				</ul>
