...
class WoocommerceIR_SMS_Settings {
	...
	public function __construct() {
		...
		add_action( 'init', array( $this, 'updateOption__3_8' ) );

		if ( is_admin() ) {

			add_action( 'admin_init', array( $this, 'adminInit' ) );
			add_action( 'admin_menu', array( $this, 'adminMenu' ), 60 );
			add_filter( 'woocommerce_settings_tabs_array', array( $this, 'adminSubMenu' ), 99999 );
			add_action( 'wp_before_admin_bar_render', array( $this, 'adminBar' ) );

			add_filter( 'pwoosms_buyer_settings', array( $this, 'buyerSettings' ) );
			add_filter( 'pwoosms_super_admin_settings', array( $this, 'superAdminSettings' ) );
			add_filter( 'pwoosms_product_admin_settings', array( $this, 'productAdminSettings' ) );
	...
	public function adminMenu() {
		...
		add_submenu_page( $slug, 'تنظیمات پیامک', 'تنظیمات پیامک',
			'manage_woocommerce', 'persian-woocommerce-sms-pro', array( $this, 'settingPage' ) );
	}
	...
	public function settingFields() {
		...
		$gateways_list = class_exists( 'WoocommerceIR_SMS_Gateways' ) ? WoocommerceIR_SMS_Gateways::get_sms_gateway() : array();
		asort( $gateways_list );
		$gateways_list = array_merge( array( 'none' => 'انتخاب کنید' ), $gateways_list );

		$shortcode = PWooSMS_Shortcode( true );

		$settings_fields = array(

			'sms_main_settings' => apply_filters( 'pwoosms_main_settings', array(
				array(
					'name'    => 'sms_gateway',
					'label'   => 'وبسرویس پیامک',
					'type'    => 'select',
			...
			'sms_super_admin_settings' => apply_filters( 'pwoosms_super_admin_settings', array(
				array(
			...
			'sms_buyer_settings' => apply_filters( 'pwoosms_buyer_settings', array(
				array(
					'name'  => 'enable_buyer',
			...
			'sms_product_admin_settings' => apply_filters( 'pwoosms_product_admin_settings', array(
				array(
			...
			'sms_notif_settings' => apply_filters( 'pwoosms_notif_settings', array(
				array(
