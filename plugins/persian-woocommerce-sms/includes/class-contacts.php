...
class WoocommerceIR_SMS_Contacts_List_Table extends WP_List_Table {
	...
	private function get_query( $count = false ) {
		...
		$order_by = ! empty( $_REQUEST['orderby'] ) ? esc_sql( trim( $_REQUEST['orderby'] ) ) : '';
...
class WoocommerceIR_SMS_Contacts {
	...
	public function createTable() {
		...
		if ( get_option( 'pwoosms_table_contacts' ) ) {
			return;
		}
		...
		update_option( 'pwoosms_table_contacts', '1' );
	}
	...
	public function contactsTable() {
		...
		if ( isset( $_GET['edit'] ) ) {
			...
			if ( ! empty( $product_id ) && $title = get_the_title( $product_id ) ) {
				echo sprintf( '<h1>مشترکین محصول "%s"</h1>', $title ) . '<br><br>';
			}
            ...
            <a class="page-title-action" href="<?php echo $add_url; ?>">افزودن مشترک جدید</a>

			<?php if ( ! empty( $_GET['product_id'] ) || isset( $_GET['add'] ) || isset( $_GET['edit'] ) ) : ?>
                <a class="page-title-action"
                   href="<?php echo remove_query_arg( array( 'product_id', 'add', 'edit' ) ); ?>">
	...
	private function editContact( $contact_id = 0 ) {
		...
		if ( ! empty( $_POST['_wpnonce'] ) ) {
			...
			$groups = ! empty( $_POST['groups'] ) ? self::prepareGroups( $_POST['groups'] ) : '';
			if ( empty( $groups ) ) {
		...
		if ( ! empty( $saved ) || ! empty( $_GET['added'] ) ) { ?>
            <div class="notice notice-success below-h2">
                <p><strong>مشترک ذخیره شد.</strong>
                    <a href="<?php echo $return_url; ?>">بازگشت به لیست مشترکین</a>
                </p>
            </div>
			<?php
		}
        ...
        <form action="<?php echo remove_query_arg( array( 'added' ) ); ?>" method="post">
            <table class="form-table">
                <tbody>
                <tr>
                    <th><label for="mobile">شماره موبایل</label></th>
                    <td><input type="text" id="mobile" name="mobile" value="<?php echo $mobile; ?>"
                ...
                <tr>
                    ...
                    <td>
						...
						foreach ( $all_groups as $group => $label ) {
							$group = strval( $group ); ?>
                            <label for="groups_<?php echo $group; ?>">
                                <input type="checkbox" name="groups[]" id="groups_<?php echo $group; ?>"
                                       value="<?php echo $group; ?>" <?php checked( in_array( $group, $contact_groups ) ) ?>>
								<?php
								echo $label;
            ...
            <p class="submit">
                <input name="submit" class="button button-primary" value="<?php echo $title; ?>" type="submit">
                <a href="<?php echo $return_url; ?>" class="button button-secondary">بازگشت</a>
				...
				<?php if ( ! empty( $contact_id ) ) :
                    ...
                    <a class="delete" href="<?php echo $delete_url; ?>" style="text-decoration: none; color: red">حذف
                        این مشترک</a>
				<?php endif; ?>
            </p>
