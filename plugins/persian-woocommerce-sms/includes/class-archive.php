...
class WoocommerceIR_SMS_Archive_List_Table extends WP_List_Table {
	...
	private function get_query( $count = false ) {
		...
		if ( ! empty( $_REQUEST['id'] ) ) {
			$post_id = $_REQUEST['id'];
			$post_id = ! is_array( $post_id ) ? explode( ',', (string) $post_id ) : $post_id;
...
class WoocommerceIR_SMS_Archive {
	...
	public function archiveTable() {
		...
		echo '<span class="description">پیامک هایی که بعد از نصب نسخه ۴ افزونه ارسال شدند، اینجا نمایش داده خواهند شد.</span>';
		...
		<?php if ( ! empty( $_GET['id'] ) ) : ?>
            <a class="page-title-action" href="<?php echo remove_query_arg( array( 'id' ) ); ?>">بازگشت به لیست آرشیو
                همه پیامک ها</a>
		<?php endif; ?>
