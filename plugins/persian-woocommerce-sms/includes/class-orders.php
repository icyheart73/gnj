...
class WoocommerceIR_SMS_Orders {
	...
	public function __construct() {
		...
		if ( $this->enabled_buyers || $this->enable_super_admin_sms || $this->enable_product_admin_sms ) {
			...
			add_filter( 'woocommerce_checkout_fields', array( $this, 'mobileLabel' ), 0 );
			add_filter( 'woocommerce_billing_fields', array( $this, 'mobileLabel' ) );

			add_action( 'wp_enqueue_scripts', array( $this, 'checkoutScript' ) );
			add_action( 'woocommerce_after_order_notes', array( $this, 'checkoutFields' ) );
			add_action( 'woocommerce_checkout_process', array( $this, 'checkoutFieldsValidation' ) );
			add_action( 'woocommerce_checkout_update_order_meta', array( $this, 'saveSmsOrderMeta' ) );

			/*بعد از تغییر وضعیت سفارش*/
			add_action( 'woocommerce_order_status_changed', array( $this, 'sendOrderSms' ), 99, 3 );

			/*بعد از ثبت سفارش*/
			add_action( 'woocommerce_checkout_order_processed', array( $this, 'sendOrderSms' ), 99, 1 );
			add_action( 'woocommerce_process_shop_order_meta', array( $this, 'sendOrderSms' ), 999, 1 );

			/*جلوگیری از ارسال بعد از ثبت مجدد سفارش از صفحه تسویه حساب*/
			add_action( 'woocommerce_resume_order', function () {
				remove_action( 'woocommerce_checkout_order_processed', array( $this, 'sendOrderSms' ), 99 );
			} );

			add_filter( 'woocommerce_form_field_pwoosms_multiselect', array(
				'WoocommerceIR_SMS_Helper',
				'multiSelectAndCheckbox'
			), 11, 4 );
			add_filter( 'woocommerce_form_field_pwoosms_multicheckbox', array(
				'WoocommerceIR_SMS_Helper',
				'multiSelectAndCheckbox'
			), 11, 4 );
			...
			if ( is_admin() ) {
				...
				add_action( 'woocommerce_admin_order_data_after_order_details', array( $this, 'changeSmsTextJS' ) );
	...
	public function checkoutScript() {
		...
		if ( PWooSMS()->Options( 'allow_buyer_select_status' ) && is_checkout() ) {
			...
			wp_register_script( 'pwoosms-frontend-js', PWOOSMS_URL . '/assets/js/multi-select.js', array( 'jquery' ), PWOOSMS_VERSION, true );
	...
	public function checkoutFieldsValidation() {
		...
		if ( ! empty( $_POST[ $mobile_meta ] ) ) {
			$_POST[ $mobile_meta ] = PWooSMS()->modifyMobile( $_POST[ $mobile_meta ] );
		} else {
			$_POST[ $mobile_meta ] = '';
		}
		...
		if ( $buyer_selected && ! PWooSMS()->validateMobile( $_POST[ $mobile_meta ] ) ) {
	...
	public function buyerSmsDetails( $order ) {

		if ( ! $this->enabled_buyers || count( PWooSMS()->GetBuyerAllowedStatuses() ) < 0 ) {
			return;
		}

		$order_id = PWooSMS()->OrderId( $order );
		...
		if ( PWooSMS()->maybeBool( get_post_meta( $order_id, '_force_enable_buyer', true ) ) ) {
			echo '<p>مشتری حق انتخاب دریافت یا عدم دریافت پیامک را ندارد.</p>';
		} else {
			$want_sms = get_post_meta( $order_id, '_buyer_sms_notify', true );
			echo '<p>آیا مشتری مایل به دریافت پیامک هست : ' . ( PWooSMS()->maybeBool( $want_sms ) ? 'بله' : 'خیر' ) . '</p>';
		}

		echo '<p>';
		if ( PWooSMS()->maybeBool( get_post_meta( $order_id, '_allow_buyer_select_status', true ) ) ) {
			...
			if ( ! empty( $allowed_status ) ) {
				...
				echo implode( ' - ', array_values( $allowed_status ) );
			}
			*/
		}
		echo '</p>';
	...
	public function sendOrderSms( $order_id, $old_status = '', $new_status = 'created' ) {
		...
		if ( current_action() == 'woocommerce_process_shop_order_meta' ) {
			...
			remove_action( 'woocommerce_process_shop_order_meta', array( $this, 'sendOrderSms' ), 999 );
		}
		...
		//customer
		$order_page = ! empty( $_POST['is_shop_order'] ) && $_POST['is_shop_order'] == 'true';

		if ( ( $order_page && ! empty( $_POST['sms_order_send'] ) ) || ( ! $order_page && $this->buyerCanGetSMS( $order_id, $new_status ) ) ) {

			$mobile  = PWooSMS()->buyerMobile( $order_id );
			$message = isset( $_POST['sms_order_text'] ) ? esc_textarea( $_POST['sms_order_text'] ) : PWooSMS()->Options( 'sms_body_' . $new_status );
	...
	public function changeSmsTextJS( $order ) {

		$order_id = PWooSMS()->OrderId( $order );

		if ( $this->enabled_buyers && PWooSMS()->validateMobile( PWooSMS()->buyerMobile( $order_id ) ) ) { ?>
	...
	public function changeSmsTextCallback() {
		...
		if ( ! empty( $_POST['order_id'] ) ) {
			...
			echo '<textarea id="sms_order_text" name="sms_order_text" style="width:100%;height:120px;"> ' . $message . ' </textarea>';
	...
	public function buyerCanGetSMS( $order_id, $new_status ) {
