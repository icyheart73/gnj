...
class WoocommerceIR_SMS_Subscription extends WP_Widget {
	...
	public function __construct() {
		...
		add_shortcode( PWooSMS_Shortcode( true, true ), array( $this, 'displayForm' ) );

		$this->enable_notification = PWooSMS()->Options( 'enable_notif_sms_main' );

		if ( $this->enable_notification ) {
			add_action( 'woocommerce_product_thumbnails', array( $this, 'showInSingleProduct' ), 100 );
			add_action( 'woocommerce_single_product_summary', array( $this, 'showInSingleProduct' ), 39 );
			add_action( 'wp_ajax_wc_sms_save_notification_data', array( $this, 'updateSubscription' ) );
			add_action( 'wp_ajax_nopriv_wc_sms_save_notification_data', array( $this, 'updateSubscription' ) );
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance = ! empty( $old_instance ) && is_array( $old_instance ) ? $old_instance : array();
	...
	public function displayForm( $product = '' ) {
        ...
        <form class="sms-notif-form" id="sms-notif-form-<?php echo $id; ?>" method="post">
            ...
            <div class="sms-notif-enable-p" id="sms-notif-enable-p-<?php echo $id; ?>">
                <label id="sms-notif-enable-label-<?php echo $id; ?>" class="sms-notif-enable-label"
                       for="sms-notif-enable-<?php echo $id; ?>">
                    <input type="checkbox" id="sms-notif-enable-<?php echo $id; ?>" class="sms-notif-enable"
            ...
            <div class="sms-notif-content" id="sms-notif-content">
				<?php foreach ( $groups as $code => $text ) : ?>
                    ...
                    <label class="sms-notif-groups-label sms-notif-groups-label-<?php echo $code; ?>"
                           for="sms-notif-groups-<?php echo $code . '_' . $id; ?>">
                        <input type="checkbox"
                               id="sms-notif-groups-<?php echo $code . '_' . $id; ?>" <?php echo $disabled; ?>
                ...
                <div class="sms-notif-mobile-div">
                    <input type="text" id="sms-notif-mobile-<?php echo $id; ?>" class="sms-notif-mobile"
                           name="sms_notif_mobile"
                           value="<?php echo get_user_meta( get_current_user_id(), PWooSMS()->buyerMobileMeta(), true ); ?>"
                           style="text-align: left; direction: ltr" <?php echo $disabled; ?>
                           title="شماره موبایل" placeholder="شماره موبایل"/>
                </div>

				<?php if ( ! $can_be_subscribe ) : ?>
                    <p id="sms-notif-disabled-<?php echo $id; ?>" class="sms-notif-disabled">
						<?php echo PWooSMS()->getValue( 'notif_only_loggedin_text', $product_id ); ?>
                    </p>
				<?php else : ?>
                    <button id="sms-notif-submit-<?php echo $id; ?>"
                ...
                <p id="sms-notif-result-p-<?php echo $id; ?>" class="sms-notif-result-p">
                    <span id="sms-notif-result-<?php echo $id; ?>" class="sms-notif-result"></span>
                </p>
            </div>
        </form>
	...
	public function updateSubscription() {
		...
		$mobile = isset( $_POST['sms_mobile'] ) ? PWooSMS()->modifyMobile( $_POST['sms_mobile'] ) : '';
		if ( empty( $mobile ) ) {
		...
		if ( ! empty( $contact['id'] ) ) {
			...
			$old_groups = ! empty( $contact['groups'] ) ? explode( ',', $contact['groups'] ) : array();
			$new_groups = array_merge( $old_groups, $groups );

			$update = WoocommerceIR_SMS_Contacts::updateContact( array(
				'id'         => $contact['id'],
				'product_id' => $product_id,
				'mobile'     => $mobile,
				'groups'     => $new_groups,
			...
			$insert = WoocommerceIR_SMS_Contacts::insertContact( array(
				'product_id' => $product_id,
				'mobile'     => $mobile,
				'groups'     => $groups,
			) );
