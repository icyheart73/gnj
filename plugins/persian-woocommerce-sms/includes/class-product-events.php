...
class WoocommerceIR_SMS_Product_Events {
	...
	public function init() {
		...
		$action = ! empty( $_POST['action'] ) ? str_ireplace( 'woocommerce_', '', $_POST['action'] ) : '';
		if ( in_array( $action, array( 'add_variation', 'link_all_variations' ) ) ) {
			return;
		}

		/*onSale*/
		add_action( 'woocommerce_process_product_meta', array( $this, 'smsIsOnSale' ), 9999, 1 );
		add_action( 'woocommerce_update_product_variation', array( $this, 'smsIsOnSale' ), 9999, 1 );
		add_action( 'woocommerce_sms_send_onsale_event', array( $this, 'smsIsOnSale' ) );
		/*inStock*/
		add_action( 'woocommerce_product_set_stock_status', array( $this, 'smsInStock' ) );
		add_action( 'woocommerce_variation_set_stock_status', array( $this, 'smsInStock' ) );
		/*outStock*/
		add_action( 'woocommerce_product_set_stock_status', array( $this, 'smsOutStock' ) );
		add_action( 'woocommerce_variation_set_stock_status', array( $this, 'smsOutStock' ) );
		/*lowStock*/
		add_action( 'woocommerce_low_stock', array( $this, 'smsIsLowStock' ) );
		add_action( 'woocommerce_product_set_stock', array( $this, 'smsIsLowStock' ) );
		add_action( 'woocommerce_variation_set_stock', array( $this, 'smsIsLowStock' ) );
	}
	...
	public function smsIsOnSale( $product_id = 0 ) {
		...
		if ( is_array( $product_id ) ) {
			return array_map( array( $this, __FUNCTION__ ), $product_id );
		}
			...
			if ( ! $product->is_on_sale() ) {
				...
				if ( ! $is_schedule ) {
					...
					if ( ! empty( $date_from ) && $date_from > strtotime( 'NOW', current_time( 'timestamp' ) ) ) {
						wp_schedule_single_event( $date_from + 3600, $schedule, array( $product_id ) );
					}
				}
