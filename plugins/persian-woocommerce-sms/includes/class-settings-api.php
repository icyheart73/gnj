...
class WoocommerceIR_SMS_Settings_Api {
	...
	function callback_html( $args ) {
		echo sprintf( '<div class="description" ' . $this->desc_style . '> %s</div>', $args['desc'] );
	}
	...
	function callback_wysiwyg( $args ) {
		...
		echo '</div>';
		echo sprintf( '<br><div class="description" ' . $this->desc_style . '> %s</div>', $args['desc'] );
	}
	...
	function show_navigation() {
		...
		foreach ( $this->settings_sections as $form ) {
			...
			$url  = add_query_arg( array( 'tab' => str_ireplace( array( 'sms_', '_settings' ), '', $form['id'] ) ) );
			...
			$html .= sprintf( '<a href="%1$s" class="nav-tab %3$s" id="%1$s-tab">%2$s</a>', $url, $form['title'], $class );
		}
		$html .= '</nav>';
		echo $html;
	}
