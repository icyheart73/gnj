 ...
 * @return  string
 */
if (!function_exists('gnje_get_excerpt')) {
    function gnje_get_excerpt($length = 55)
    {
        $post = get_post(null);
...
/**
 * Numeric pagination for GNJE widget
 *
 ...
 * @return  string
 */
if( ! function_exists( 'gnje_pagination' ) ) {
    function gnje_pagination(  $range = 2, $current_query = '', $pages = '', $prev_icon='<i class="cs-font ganje-icon-arrow-left"></i>', $next_icon='<i class="cs-font ganje-icon-arrow-right"></i>' ) {
        $showitems = ($range * 2)+1;
        ...
        if(1 != $pages) { ?>
            <div class="gnje-pagination clearfix">
                <?php if ( $paged > 1 ) { ?>
                    <a class="gnje-pagination-prev gnje_pagination-item" href="<?php echo esc_url(get_pagenum_link($paged - 1)) ?>"><?php echo $prev_icon?></a>
                <?php }

                for ($i=1; $i <= $pages; $i++) {
                    if (1 != $pages &&( !($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems )) {
                        if ($paged == $i) { ?>
                            <span class="current gnje_pagination-item"><?php echo esc_html($i) ?></span>
                        <?php } else { ?>
                            <a href="<?php echo esc_url(get_pagenum_link($i)) ?>" class="inactive gnje_pagination-item"><?php echo esc_html($i) ?></a>
                            <?php
                        }
                    }
                }
                if ($paged < $pages) { ?>
                    <a class="gnje-pagination-next gnje_pagination-item" href="<?php echo esc_url(get_pagenum_link($paged + 1)) ?>"><?php echo $next_icon?></a>
                <?php } ?>
            </div>
            <?php
        }
...
if ( ! function_exists( 'gnje_scrape_instagram' ) ) {
    function gnje_scrape_instagram( $username ) {
...
if ( ! function_exists( 'gnje_images_only' ) ) {
    function gnje_images_only( $media_item ) {
...
if ( ! function_exists( 'gnje_abbreviate_total_count' ) ) {
    function gnje_abbreviate_total_count( $value, $floor = 0 ) {
        if ( $value >= $floor ) {
...
if ( ! function_exists( 'gnje_time_elapsed_string' ) ) {
    function gnje_time_elapsed_string($datetime, $full = false) {
        $now = new DateTime;
 ...
 */
if (!function_exists('zoo_oembed_dataparse')||!function_exists('gnje_oembed_dataparse')) {
	function gnje_oembed_dataparse($return, $data, $url)
	{
...
add_filter('oembed_dataparse', 'gnje_oembed_dataparse', 10, 3);
