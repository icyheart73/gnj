...
*/
if(!function_exists('gnje_ajax_product_filter')){

    function gnje_ajax_product_filter(){
    ...
    add_action('wp_ajax_gnje_ajax_product_filter', 'gnje_ajax_product_filter');
    add_action( 'wp_ajax_nopriv_gnje_ajax_product_filter', 'gnje_ajax_product_filter' );
}
...
if(!function_exists('zoo_sold_bar')) {
    function zoo_sold_bar() {
        ...
        <div class="sold-bar <?php echo esc_attr($parse_class); ?>">
            ...
            <h4 class="sold-label">
                <?php echo esc_html__('Only','gnje'); ?>
                <span><?php echo esc_attr($result['stock']); ?></span>
                <?php echo esc_attr($result['stock']) > 1 ? esc_html__('items ','gnje') : esc_html__('item ','gnje');?>
                <?php echo esc_html__('left in stock!','gnje'); ?>
            </h4>
            <?php else: ?>
            <h4 class="sold-label">
                <?php echo esc_html__('Buy','gnje'); ?>
                <span><?php echo esc_attr($result['stock']); ?></span>
                <?php echo esc_attr($result['stock']) > 1 ? esc_html__('items ','gnje') : esc_html__('item ','gnje');?>
                <?php echo esc_html__(' in stock!','gnje'); ?>
            </h4>
            <?php endif; ?>
            ...
            <div class="sold-bar-count">
                <span><?php echo esc_html__('Sold: ','gnje'); ?><label><?php  echo esc_attr($result['sold']) .'/'.esc_attr($result['stock'] + $result['sold']); ?></label></span>
            </div>
            <?php endif; ?>
        </div>
        <?php
