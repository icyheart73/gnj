<?php
/**
 * View template for Ganje Product Advanced.
 *
 * @package GNJE\Templates
 * @copyright 2018 GanjeSoft. All rights reserved.
 */

$product_ids = $default_asset = $filter_arr = $gnje_json_config = '';
...
<?php if($settings['pagination'] != 'none' && $settings['tabs_filter'] == 'none' && $settings['layout'] == 'grid') : ?>
  ...
  <?php if($zoo_pag_type == 'ajaxload' || $zoo_pag_type == 'infinity'): wp_enqueue_script('infinite-scroll'); ?>
    ...
    <?php endif; ?>
    <?php
    gnje_pagination(3, $product_query, '', '<i class="zoo-icon-long-arrow-left"></i> ' . esc_html__('', 'gnje'), esc_html__('', 'gnje') . ' <i class="zoo-icon-long-arrow-right"></i>');
    ?>
    <?php if($zoo_pag_type == 'ajaxload' || $zoo_pag_type == 'infinity') : ?>

        <div class="scroller-status">
            <div class="infinite-scroll-request">
                <div class="pagination-loading"><span class="loading"><?php echo esc_html__('LOADING...','gnje') ?></span></div>
            </div>
            <p class="infinite-scroll-last"><?php echo esc_html__('All items loaded','gnje'); ?></p>
            <p class="infinite-scroll-error"><?php echo esc_html__('No more page','gnje'); ?></p>
        </div>
        <?php if ($zoo_pag_type == 'ajaxload') { ?>
            <p class="view-more-button"><span><?php echo esc_html__('Load More','gnje') ?></span></p>
        <?php } ?>
    </div>
<?php endif; ?>
<?php endif; ?>
