...
class Gnj_Otp_Handler{
	...
	public function __construct(){
	    add_action('devmode' , array($this , 'testOtp'));
		self::$ip_address 	= getUserIP();
		self::cleanup();
	}
	...
	*/
	public static function onlySendOTPSMS( $phone_code, $phone_no ){
        ...
        if ( !PWOOSMS_VERSION ) return false;
	...
	*/
	...
	public static function set_otp_data( $key, $value = '' ){
		...
		update_option( 'gnj_otp_users', $users );
	}


	public static function get_otp_users(){
		return (array) get_option( 'gnj_otp_users' );
	}

	public static function get_otp_data(){
		$users = get_option( 'gnj_otp_users' );
	...
	public static function ok_to_send_otp( $phone_code = '', $phone_no = '' ){
		...
		$time_passed = strtotime("now") - $data['created'];

		if( $data['sent_times'] > $resend_limit ){
			$unban_time_left = 600 - $time_passed;
	...
	public static function getOTPSMSText( $otp ){
		...
		$sms_text = apply_filters( 'gnj_phone_sms_text',$sms_text );
	...
	*/
	public static function cleanup(){
		...
		$last_cleanup = (int) get_option( 'gnj_last_cleanup', true );

		$now = time();

		//Check if cleanup was done 24 hours ago
		if( ( $now - $last_cleanup ) < ( apply_filters( 'gnj_cleanup_last_done', 24 ) * 1 ) ){
			return;
		}
		...
		update_option( 'gnj_otp_users', $users );
		update_option( 'gnj_last_cleanup', $now );

	}
}

function gnj_otp_handler(){
	return Gnj_Otp_Handler::get_instance();
}
gnj_otp_handler();
