...
abstract class CleverWidgetBase extends Widget_Base {
	...
	protected function getListRevSlider() {
		...
		if ( $arrSliders ) {
			$revsliders['0'] = esc_html__( 'Select slider', 'cafe' );
			foreach ( $arrSliders as $slider ) {
				$revsliders[ $slider->getAlias() ] = $slider->getTitle();
			}
		} else {
			$revsliders['0'] = esc_html__( 'No sliders found', 'cafe' );
		}
	...
	protected function getViewTemplate( $tpl_slug, $tpl_name, $settings = [] ) {
		...
		if ( $tpl_name ) {
			$templates[] = 'clever-addons-for-elementor/templates/' . $tpl_slug . '-' . $tpl_name . '.php';
			$templates[] = 'clever-addons-for-elementor/templates/' . $tpl_slug . '/' . $tpl_name . '.php';
			$templates[] = 'src/templates/' . $tpl_slug . '-' . $tpl_name . '.php';
			$templates[] = 'src/templates/' . $tpl_slug . '/' . $tpl_name . '.php';
		}

		$templates[] = 'clever-addons-for-elementor/' . $tpl_slug . '.php';
		...
		foreach ( $templates as $template ) {
			if ( file_exists( STYLESHEETPATH . '/' . $template ) ) {
				...
				break;
			} elseif ( file_exists( WP_PLUGIN_DIR . '/clever-addons-for-elementor/' . $template ) ) {
				$located = WP_PLUGIN_DIR . '/clever-addons-for-elementor/' . $template;
				break;
			} else {
				$located = false;
			}
		...
		if ( $located ) {
			...
			throw new InvalidArgumentException( sprintf( __( 'Failed to load template with slug "%s" and name "%s".', 'cafe' ), $tpl_slug, $tpl_name ) );
		}
	}
	...
	protected function getImgSizes() {
		...
		endforeach;
		$output['full'] = esc_html__( 'Full', 'cafe' );
